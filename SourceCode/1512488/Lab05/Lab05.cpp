...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE :
	{
		...
		//Tree View
		g_hTree = TaoCay(extStyle, hWnd, IDT_TREEVIEW, g_hInstance, x, y, nWidth, nHeight, style);

		loadToTree(g_D, g_hTree);
		SetFocus(g_hTree);
		...
		g_hList = TaoDS(extStyle, hWnd, IDL_LISTVIEW, g_hInstance, x, y, nWidth, nHeight, style);
		loadToList(g_D, g_hList);
	...
	case WM_NOTIFY:
	{
		...
		case TVN_ITEMEXPANDING: 
			...
			loadChild(currSelected, g_hTree);
			break;
		case TVN_SELCHANGED:
			currSelected = TreeView_GetSelection(g_hTree); 
			TreeView_Expand(g_hTree, currSelected, TVE_EXPAND);

			ListView_DeleteAllItems(g_hList);
			loadListItemAt(getPath(currSelected, g_hTree), hWnd, g_hList, g_D);
			break;
		...
		case NM_DBLCLK:
			...
			if (notifyMess->hwndFrom == g_hList)
				loadOrExecSelected(g_hList);
			break;
		}
...
void loadToTree(TroGiup *d, HWND m_hTree)
{
	...
	for (int i = 0; i < g_D->getCount(); ++i)
	{
		tv_Insert.hParent = hMyComputer; 
		tv_Insert.item.iImage = driveIconIndex;
		tv_Insert.item.iSelectedImage = driveIconIndex;
		tv_Insert.item.pszText = g_D->getDisplayName(i); 
		tv_Insert.item.lParam = (LPARAM)g_D->getLetter(i);
...
void loadOrExecSelected(HWND m_hList)
{
	...
	if (GetFileAttributesEx(FPath, GetFileExInfoStandard, &fd) != 0)
	{
		if (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
		{
			...
			loadDirItemToLisview(g_hWnd, m_hList, FPath);
		}
		else 
		{
...
void loadListItemAt(LPCWSTR path, HWND m_hParent, HWND m_hList, TroGiup *d)
{
	...
	else if (_tcscmp(path, _T("MyComputer")) == 0)
	{
		...
		loadDirItemToLisview(g_hWnd, m_hList, path);
}
